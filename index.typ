...
#strong[SDT] uses the built-in #strong[Julia] package manager to keep all dependencies up to date. Furthermore, we use strict semantic versioning: major versions correspond to changes that would break user-developped code; minor versions represent additional functionalities; patch releases cover minor bug fixes or documentation changes. All component packages are versioned independently, and have their own #emph[CHANGELOG] file documenting each release. This strict reliance on semantic versioning removes the issues of maintaining compatibility when new functionalities are added: all releases in the #emph[v1.x.x] branch of #strong[SDT] depend on component packages in their respective #emph[v1.x.x] branch, and users can benefit from new functionalities without needing to adapt existing code. This behavior is extensively tested, both through unit tests and through integration testing generated as part of the online documentation. Following a constructive cost model analysis @Kemerer1987 of the version described in this publication, the package represents approx. 11k lines of active code (no blank lines, no comments), for an estimated development cost of approx. 325k USD.

The #strong[SDT] package benefits from close integration with other packages in the #strong[Julia] universe. Notably, this includes #strong[Makie] \[including #strong[GeoMakie];; #cite(<Danisch2021>, form: "prose");\] for plotting and interactive data visualisation: all relevant plot types are overloaded for layer and occurrence data. Most data handled by #strong[SDT] can be exported using the #strong[Tables] interface, which allows data to be consumed by other packages like #strong[DataFrames] @Bouchet-Valat2023 and #strong[MLJ] @Blaom2020, or directly saved as csv files. Interfaces to internal #strong[Julia] methods are implemented whenever they are pertinent. #strong[SimpleSDMLayers] and #strong[OccurrencesInterface] objects behave like arrays, are iterable, and broadcastable. The #strong[SDeMo] package relies in part on the #strong[StatsAPI] interface, allowing to easily define new data transformation and classifier types to support additional features. Achieving integration with other packages through method overloading and the adherence to well-established interfaces is important, as it increases the chances that additional functionalities external to #strong[SDT] can be used directly or fully supported with minimal addition of code.

#figure([
#box(image("figures/structure.drawio.png"))
], caption: figure.caption(
position: bottom, 
[
Overview of the packages included in #strong[SpeciesDistributionToolkit];. The packages are color-coded by intended use (acquisition, representation, and analysis of data). The specific content of each package is presented in the main text. Note that because the package relies on #emph[interfaces] to facilitate code interoperability, there are only three dependency relationships (black arrows). Some packages can interact with data sources, represented on the left side of the figure. When loading #strong[SpeciesDistributionToolkit];, all public methods from the package are accessible to the user.
]), 
...
The #strong[SimpleSDMLayers] package offers a series of types to represent raster data in arbitrary projections defined by a proj string @Evenden2024. This package provides the main data representation for most spatial functionalities that #strong[SDT] supports, and handles saving and loading data. It also contains utility functions to deal with raster data, including interpolation to different spatial grids and CRS, rescaling and quantization of data, masking, and most mathematical operations that can be applied to rasters.
...
#strong[Fauxcurrences] is inspired by the work of #cite(<Osborne2022>, form: "prose");, and allows generating a series of simulated occurrence data that have the same statistical structure as observed ones. The package supports multi-species data, with user-specified weights for conserving intra and inter-specific occurrence distances.

Finally, #strong[SDeMo] provides tools for training and education on species distribution modeling. By providing a series of data transformation (PCA, Whitening, z-score) and classifiers (currently BIOCLIM, Naive Bayes, logistic regression, and decision trees), it offers the basic elements to demonstrate training and evaluation of SDMs, as well as techniques related to heterogeneous ensembles and bagging with support for arbitrary consensus @Marmion2009 and voting @Drake2014 functions. #strong[SDeMo] promotes the use of interpretable techniques: the package supports regular @Elith2005 and inflated @Zurell2012 partial responses, as well as the calculation and mapping of Shapley values @Wadoux2023@Mesgaran2014 using the standard Monte-Carlo approach @Mitchell2021. Counterfactuals @VanLooveren2019@Karimi2019, representing perturbation of the input data leading to the opposite prediction (#emph[i.e.] "what environmental conditions would lead to the species being absent") can also be generated.
...
#strong[SimpleSDMDatasets] uses local storage of raster data for future use, to avoid re-downloading data upon repeated use. The location of the data is (i) standardized by the package itself, making the file findable to humans, and (ii) changeable by the user to, #emph[e.g.];, store the data within the project folder rather than in a central location. As much as possible, #strong[SDT] will only read the part of the raster data that is required given the region of interest to the user. This is done by providing additional context in the form of a bounding box (in WGS84, regardless of the underlying raster data projection, in line with the GeoJSON specification). #strong[SDT] has methods to calculate the bounding box for all the objects it supports.

== Using data from GBIF
<using-data-from-gbif>
#strong[SDT] provides strong integration between data on species occurrences and source of geospatial information. To illustrate this, we will collect data on the distribution of #emph[Akodon montensis] (Rodentia, family Cricetidae), a known host of orthohantaviruses @Burgos2021@Owen2010, in Paraguay. In Supp. Mat. 2 we (i) request occurrence data using the #strong[GBIF] package, (ii) download the silhouette of the species through #strong[Phylopic];, and (iii) extract temperature and precipitation data at the points of occurrence based on bioclimatic data layers. The results are presented in @fig-gbif-phylopic. The full notebook includes information about basic operations on raster data, as well as extraction of data based on occurrence records.
...
<species-and-location-contribution-to-beta-diversity>
In the final case study (Supp. Mat. 4), we simulate the distribution of virtual species @Hirzel2001 with a logistic response to two environmental covariate @Leroy2016. We then use this simulated sample to perform the decomposition of $beta$-diversity introduced by #cite(<Legendre2013>, form: "prose") and applied by #cite(<Dansereau2022>, form: "prose") to spatially continuous data. This simulates the potential distribution of hotspots and coldspots of ecological uniqueness. The results are presented in @fig-virtual-species.
...
position: bottom, 
[
Virtual distribution of normalized (mean of 0 and unit variance) locality contribution to beta-diversity @Legendre2013, based on a pool of 100 virtual species. The inset histogram represents the standardized species contribution to beta-diversity. Red areas represent comparatively more unique areas in terms of simulated species composition. The code to produce this ﬁgure is available as Supp. Mat. 4.
]), 
kind: "quarto-float-fig", 
supplement: "Figure", 
)
