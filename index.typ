...
= Case studies
<case-studies>
In this section, we provide a series of case studies to illustrate the use of the package. The on-line documentation offers longer tutorials, as well as a series of how-to vignettes to illustrate the full scope of what the package allows. The situation we use for most case studies is the distribution of #emph[Akodon montensis] (Rodentia, family Cricetidae), a known host of orthohantaviruses @Burgos2021@Owen2010, in Paraguay. As the notebooks accompanying this article cover the full code required to run these case studies, we do not present code snippets in the main text (as they are presented with detailed explanations in the Supp. Mat.), but rather focus on explaining how the component packages work together in each example.

== Using data from GBIF
<using-data-from-gbif>
#strong[SDT] provides strong integration between data on species occurrences and source of geospatial information. in Supp. Mat. 1, we (i) request occurrence data using the #strong[GBIF] package, (ii) download the silhouette of the species through #strong[Phylopic];, and (iii) extract temperature and precipitation data at the points of occurrence. The results are presented in @fig-gbif-phylopic. The full notebook includes information about basic operations on raster data, as well as extraction of data based on occurrence records.
...
<distribution-of-a-virtual-species>
In the final case study (Supp. Mat. 4), we simulate a the distribution of a virtual species @Hirzel2001 with a logistic response to each environmental covariate @Leroy2016, and a prevalence similar to the one predicted in @fig-sdm-output. The results are presented in @fig-virtual-species.
