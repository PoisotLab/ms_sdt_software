...
position: bottom, 
[
Predicted range of #emph[Akodon montensis] in Paraguay based on a rotation forest trained on GBIF occurrences and the BioClim variables. The code to produce this ﬁgure is available as Supp. Mat. 3.
]), 
kind: "quarto-float-fig", 
supplement: "Figure", 
)
...
Plans for active development of the package are focused on (i) additional techniques for pseudo-absence generations, likely leading to their separate component package, (ii) full compatibility with the #strong[MultivariateStatistics] and #strong[Clustering] packages for transformation and aggregation, and (iii) additional #strong[SDeMo] functionalities to allow cross-validation techniques with biological relevant structure @Roberts2017.

#strong[Acknowledgements:] TP is funded by an NSERC Discovery grant, a Discovery Acceleration Supplement grant, and a Wellcome Trust grant (223764/Z/21/Z).
